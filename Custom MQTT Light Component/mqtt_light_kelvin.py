"""
MQTT LIGHT KELVIN
A slight modification of MQTT Lights
I've commented the modifications below with the tag #MODIFICATION

drop this into *YourConfigDir*\custom_components\light
Last tested with .77.3
"""
import logging
...
class MqttLight(MqttAvailability, Light):
    ...
    async def async_added_to_hass(self):
        ...
        def color_temp_received(topic, payload, qos):
            ...
            #MODIFICATION - hijacks the paylod - converts to integer so that we can then take the mired value and convert back to kelvin
            mired = int(templates[CONF_COLOR_TEMP](payload))
            self._color_temp = int(1000000/float(mired))
            self.async_schedule_update_ha_state()
    ...
    async def async_turn_on(self, **kwargs):
        ...
        if ATTR_COLOR_TEMP in kwargs and \
           self._topic[CONF_COLOR_TEMP_COMMAND_TOPIC] is not None:
            #MODIFICATION - Converts the Mired to Kelvin
            color_temp = int((1000000/float(kwargs[ATTR_COLOR_TEMP])))
            mqtt.async_publish(
